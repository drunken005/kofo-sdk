...
function Kofo() {
    ...
    this.config = config;
    let mqService = new MqService(config);
...
Kofo.prototype._messageSchedule = function (message) {
    let messageProvider = MessageProvider.init(message.chain, message.currency, this._event, this.config);
    let messageHandler = Util.messageType(message.type, messageProvider);
    if (!messageHandler) {
        this._logger.error(`clientId=${this._mqService.clientId}||onMessageError()||InvalidType=${message.type}`);
        return;
    }
    messageHandler.call(messageProvider, message).catch(err => {
        this._logger.error(`clientId=${this._mqService.clientId}||onMessageHandler()||${MqService.errInfo(err)}`);
    });
};
...
Kofo.prototype.signatureCallback = function (type, chain, currency, settlementId, signedObj) {
    let callbackProvider = CallbackProvider.init(chain, currency, this._event, this.config);
