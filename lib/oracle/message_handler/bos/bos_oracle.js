...
/**
 * Provides BOS with Hash lock, Withdraw, Refund, Approve signature callback handler
 */
class BosOracle extends Oracle {
    constructor() {
    ...
    async submitHashLockTxHandler(message) {
        ...
        const _self = this;
        _self.logger.info(`BOS ${roleEnum} submitHashLockTx ${settlementId}`);
        ...
        if (!hasLocker) {
            try {
                ...
                const {lockId, signHashList, rawTransaction, nlockDate} = lockTx;
                if (!hasLockTxHash) {
                ...
                _statusNotice_(NOTICE_TYPE.FAIL_HASH_LOCK, {lockTx, lockTxHash: lockTxHash.txHash});
                throw  err;
            }
        } else {
    ...
    async submitWithdrawTxHandler(message) {
        ...
        _self.logger.info(`BOS ${roleEnum} submitWithdrawTx ${settlementId}`);
    ...
    async submitRefundTxHandler(message) {
        ...
        _self.logger.info(`BOS ${roleEnum} submitRefundTx ${settlementId}`);
